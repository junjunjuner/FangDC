...
from cityhouse.settings import FIELDS_TO_EXPORT1
from cityhouse.settings import FIELDS_TO_EXPORT2
...
class CityhousePipeline1(object):
    def __init__(self):
        # 连接数据库
...
class CityhousePipeline2(object):
    def __init__(self):
        # 连接数据库
        self.client = pymongo.MongoClient(host=settings['MONGO_HOST'], port=settings['MONGO_PORT'])
        # 数据库登录需要帐号密码的话,在settings.py底部追加
...
class CSVPipeline1(object):
  ...
  def spider_opened(self, spider):
    file = open('城市房产-城市数据（二手房）.csv', 'wb')
...
class CSVPipeline2(object):
  ...
  def spider_opened(self, spider):
    file = open('城市房产-城市数据（新楼盘）.csv', 'wb')
    # file = open('%s_pages_%s.csv' % (spider.name,self.printfNow()), 'a+b')
    self.files[spider] = file
    self.exporter = CsvItemExporter(file)
